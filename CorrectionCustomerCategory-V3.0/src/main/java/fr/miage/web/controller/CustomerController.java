...
// #### V2.0 La demande des clients d'une catégorie se fait par l'url /customer/X
// #### V2.0 où X est le nom d'une catégorie. Il faut découper l'url grâce à @PathVariable
...
// #### V2.0 Le nom d'une catégorie sera récupérer dans "categoryname". L'ouverture
// #### V2.0 de /customer/A met "A" dans categoryname.
// #### V2.0 /customer met null dans categoryname.
    public String findAll(@PathVariable(required = false, name = "categoryname")String categoryname, Model model) {
        ...
        LOGGER.info("Controller Customer - " + categoryname);
        // #### V2.0 categoryname == null => /customer donc liste complète (comme avant)       
        if (categoryname == null) {        
            model.addAttribute("customers", customerService.findAll());
        } else {
            Category category = categoryService.findByName(categoryname);
            // #### V2.0 aucune catégorie de ce nom n'existe => liste vide
            if (category == null) {
                model.addAttribute("customers", null);
            } else {
            // #### V2.0 sinon => liste des clients de catégorie de ce nom
                model.addAttribute("customers", customerService.findByCategory(category));
            // #### V2.0 Pour pouvoir afficher de quelle catégorie sont les clients
